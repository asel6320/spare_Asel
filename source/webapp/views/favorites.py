        ...
        if self.part.amount > 0:
            if request.user.is_authenticated:
                favorite, created = Favorite.objects.get_or_create(
                    part=self.part, user=request.user
                )
            else:
                ...
                favorite, created = Favorite.objects.get_or_create(
                    part=self.part, session_key=session_key
                )
            ...
            return JsonResponse({"status": status, "favorite_count": favorite_count})

        return JsonResponse(
            {"status": "error", "message": "Запчасть отсутствует на складе"}, status=400
        )
...
class FavoriteView(View):
    def get(self, request, *args, **kwargs):
        ...
        latest_price_subquery = (
            PriceHistory.objects.filter(part=OuterRef("part"))
            .order_by("-date_changed")
            .values("price")[:1]
        )
        favorites_with_price = favorites.annotate(
        ...
        return render(
            request, "favorites/favorites.html", {"favorites": favorites_with_price}
        )
