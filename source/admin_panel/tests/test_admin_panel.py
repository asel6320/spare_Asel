...
class AdminPanelTests(TestCase):
    ...
    def test_create_car_brand(self):
        ...
        self.login_staff_user()
        response = self.client.post(reverse('admin_panel:model_add', args=['carbrand']), {
            'name': 'Toyota2'
        })
        self.assertEqual(response.status_code, 302)
    ...
    def test_edit_car_brand(self):
        ...
        response = self.client.post(reverse('admin_panel:model_edit', args=['carbrand', car_brand.id]), {
            'name': 'Honda Updated'
        })
        self.assertEqual(response.status_code, 302)
    ...
    def test_delete_car_brand(self):
        ...
        response = self.client.post(reverse('admin_panel:model_delete', args=['carbrand', car_brand.id]))
        self.assertEqual(response.status_code, 302)
