...
class PartsFilterForm(forms.Form):
    country = forms.ModelChoiceField(
        queryset=Country.objects.all(), required=False, label="Страна производства"
    )
    brand = forms.ModelChoiceField(
        queryset=CarBrand.objects.all(), required=False, label="Марка машины"
    )
    ...
    max_price = forms.DecimalField(
        required=False, label="Максимальная цена", min_value=0
    )
